...
def create_folder(drive_service, name: str, user_id: str, parent_id: str="root") -> Optional[str]:
  ...
  try:
    ...
    if not items:
      ...
      id = folder.get('id')
    else: id = items[0].get('id')
  except Exception as e:
    log.error("Error creating thinkledger folder: ", e)
    return None

  # Store the folder id in redis
  try: redis.set(f"{name}:{user_id}", id)
  except Exception as e:
    log.error("Error storing thinkledger folder id in redis: ", e)
    return None
  return id

def create_spreadsheet(d_service, s_service, name: str, folder_id: str, user_id: str) -> Optional[str]:
  """
...
def create_transaction_sheet(s_service, spreadsheet_id: str) -> Optional[str]:
  """
